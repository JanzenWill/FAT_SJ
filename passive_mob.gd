...
func show_hit_flash() -> void:
	if is_hit: #is false just returns right away, maybe delete?
		return

	is_hit = true
	$AnimatedSprite2D.modulate = Color(1, 0.4, 0.4) # turn red
	await get_tree().create_timer(0.1).timeout #turns red for so long
